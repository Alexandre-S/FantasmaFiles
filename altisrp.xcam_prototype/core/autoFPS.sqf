[] spawn {
	...
	while{true} do {
		...
		if(havena_ok > 0 || havena_ok < 0 || (dayz_combat == 1 && (havena_view < havena_mincombat))) then {
			...
			waituntil{sleep 0.1;((((diag_fps <= (havena_targetfps)) && (havena_view > havena_minview)) && dayz_combat == 0) OR (havena_view > havena_maxview)) OR (((diag_fps > (havena_targetfps+5)) && (havena_view < havena_maxview)) OR (havena_view < havena_minview)) OR (dayz_combat == 1 && (havena_view < havena_mincombat))};
			havena_ok = 0;
		} else {
			if(havena_debug) then {
				hint "WTF";
