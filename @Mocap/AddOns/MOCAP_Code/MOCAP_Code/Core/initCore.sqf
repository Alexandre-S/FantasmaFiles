...
if (isClient && !isHeadless) then
{
    ...
    // ----------------------------------------------
     // Keys
    ["MOCAP", "Main attack button", {if (isNil 'MOCAP_disabled_fighting') then {"hand" call MOCAP_RscDisplayMission_fnc_EH_keyDown};}, [DIK_T, [false, false, false]]] call CBA_fnc_registerKeybind;
    ["MOCAP", "Legs kick", {if (isNil 'MOCAP_disabled_fighting') then {"leg" call MOCAP_RscDisplayMission_fnc_EH_keyDown};}, [DIK_T, [true, false, false]]] call CBA_fnc_registerKeybind;
    ["MOCAP", "Takedowns/Grabs", {if (isNil 'MOCAP_disabled_grabs') then {"grab" call MOCAP_RscDisplayMission_fnc_EH_keyDown};}, [DIK_T, [false, true, false]]] call CBA_fnc_registerKeybind;
    }
    else
    {
