...
*/
private["_vehicleClass","_dbInfo","_units","_idleTime","_query","_sql","_uid","_plate","_t1","_deleted","_debugmap","_assur","_needfix","_waitspawn"];
_deleted = false;
_debugmap = true;
while {true} do
{
				...
				if(!alive _x) then {
					...
					if((_x isKindOf "LandVehicle") || (_x isKindOf "Air") || ((_x isKindOf "Ship") && !(_x isKindOf "FloatingStructure_F"))) then {
						...
						if((typeOf _x) in ["Land_BottlePlastic_V1_F","Land_TacticalBacon_F","Land_Can_V3_F","Land_CanisterFuel_F","Land_Money_F","Land_Suitcase_F","WeaponHolderSimulated","GroundWeaponHolder"]) then {
							...
							if(_idleTime == 0 && !((typeOf _x) in ["Land_CargoBox_V1_F","B_UAV_01_F","Land_CashDesk_F","RoadBarrier_small_F","RoadCone_L_F","Land_Razorwire_F","Box_IND_Grenades_F_RP","B_supplyCrate_F_RP","Land_BagFence_Long_F","Land_BagFence_Round_F"])) then {
								if(!isNil "_x" && {!isNull _x}) then {
