...
class AGM_Core_Default_Keys {
  ...
  class modifierKey {
	...
	class Surrender {
		...
		statement = "[player, true] call AGM_Captives_fnc_surrender";
	...
	class Fight {
		displayName = "Combat - Main/Arme";
		condition = "isNil 'MOCAP_disabled_fighting' && {vehicle player == player}";
		statement = "'hand' call MOCAP_RscDisplayMission_fnc_EH_keyDown";
		key = 46;
		shift = 0;
		control = 0;
		alt = 0;
	...
	class FightLegs {
		displayName = "Combat - Pied";
		condition = "isNil 'MOCAP_disabled_fighting' && {vehicle player == player}";
		statement = "'leg' call MOCAP_RscDisplayMission_fnc_EH_keyDown";
		key = 46;
		shift = 1;
		control = 0;
		alt = 0;
	...
	class FightKill {
		displayName = "Combat - Etrangler";
		condition = "isNil 'MOCAP_disabled_grabs' && {vehicle player == player}";
		statement = "'grab' call MOCAP_RscDisplayMission_fnc_EH_keyDown";
		key = 46;
		shift = 0;
		control = 1;
		alt = 0;
...
class AGM_Core_canInteractConditions {
  ...
  class AGM_Interaction_isNotDead {
	condition = "alive _player && isNil {_player getVariable 'Revive'}";
  };
  class AGM_Interaction_isNotIncapacitated {
	condition = "(animationState player) != 'Incapacitated' && {!(player getVariable ['MOCAP_lockControls', false])} && {!(player getVariable ['MOCAP_animationInProgress', false])}";
  };
};
