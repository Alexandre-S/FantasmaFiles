...
// if (_state) then {
	...
	_unit spawn {
		...
		_posATL = getPosATL _this;
		_obj = "Land_ClutterCutter_small_F" createVehicle (_posATL);
		_obj setPosATL (_posATL);
		_obj setDir (direction _this);
		_this attachTo [_obj,[0,0,0]];
		_this playMove "AmovPercMstpSsurWnonDnon";
		
		waituntil{!(_this getVariable ["AGM_isSurrender", false]) || {!(alive _this)} || {_this getVariable ["AGM_Unconscious", false]}};
		...
		if !(_this getVariable ["AGM_Unconscious", false]) then {
			[[_this,"AmovPercMstpSsurWnonDnon_AmovPercMstpSnonWnonDnon","playNow"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
			[[_this,"AmovPercMstpSsurWnonDnon_AmovPercMstpSnonWnonDnon","switch"],"life_fnc_animSync",true,false] spawn life_fnc_MP;
		} else {
			_this playMoveNow "unconscious";
		};
		// _this setVariable["attachedObject",nil,true];
