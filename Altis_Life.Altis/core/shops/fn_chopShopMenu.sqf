...
_nearVehicles = nearestObjects [getMarkerPos (_this select 3),["LandVehicle","Ship","Air"],50];
...
_control = ((findDisplay 39400) displayCtrl 39402);
{
	if(alive _x) then {
		...
		_ind = [_className,(call life_assur_prices)] call life_fnc_index;
		
		if(_ind != -1 && count crew _x == 0) then {
			_price = ((call life_assur_prices) select _ind) select 1;
