...
if((life_veh_shop select 0) == "med_air_hs") then {
	...
	if(typeName _spawnPoints == typeName []) then {
		...
		{if(count(nearestObjects[(getMarkerPos _x),["Land","Ship","Air"],5]) == 0) exitWith {_spawnPoint = _x};} foreach _spawnPoints;
	} else {
		if(count(nearestObjects[(getMarkerPos _spawnPoints),["Land","Ship","Air"],5]) == 0) exitWith {_spawnPoint = _spawnPoints};
	};
};
