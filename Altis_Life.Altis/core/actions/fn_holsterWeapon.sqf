		...
		{
			{
				if ( (getNumber(configFile >> "cfgWeapons" >> _x >> "type")) in [1,2,4] ) exitWith
				{
					...
					hint "Tu as récupéré l'arme de ton sac à dos";
				};
			} forEach _curBack;
			if (_curWep != "") exitWith {};
			
			{
				if ( (getNumber(configFile >> "cfgWeapons" >> _x >> "type")) in [1,2,4] ) exitWith
				{
					...
					player addWeapon _x;
					hint "Tu as récupéré l'arme de ta veste";
				};
			} forEach _curVest;
		};
	};
	...
	case 1:
	{
		if((player canAddItemToBackpack _curWep) || (player canAddItemToVest _curWep)) then
		{
			...
			if(player canAddItemToBackpack _curWep)then
			{
				...
				hint "Ton arme a été rangé dans ton sac";
			}else
			{
				player addItemToVest _curWep;
				hint "Ton arme a été rangé dans ta veste";
			};
		...
		}else
		{
			...
			hint "Ton arme a été rangé dans le hoster";
		};
	};
};