...
switch (_code) do
{
	...
	case 38: 
	{
		...
		if(_shift && (playerSide in [west,independent] OR (player getVariable["life_dep",false]))) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","C_Offroad_01_F_DEP_RP","B_MRAP_01_F","C_SUV_01_F","B_MRAP_01_hmg_F"]) then {
