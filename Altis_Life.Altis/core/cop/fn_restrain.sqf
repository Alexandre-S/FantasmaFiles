...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,50] call life_fnc_nearUnits) || !([civilian,getPos player,50] call life_fnc_nearUnits) || !([independent,getPos player,50] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
